...
class OctagonoGPS(models.Model):
    ...
    def _get_invoiced(self):
        ...
        for order in self:
            ...
            if order.state not in ('registered', 'done'):
                invoice_status = 'no'
    ...
    name = fields.Char(string='Nombre de Referencia', compute='_compute_name', store=True)
    ...
    installer_id = fields.Many2one('hr.employee', "Instalador", domain="[('department_id.name', '=', 'Operaciones')]",
                                   required=True)
    ...
    #         _logger.info('name_get ====> %s', res)
    #         return res
    #     return super(OctagonoGPS, self).name_get()
    #
    # @api.model
    # def name_search(self, name='', args=None, operator='ilike', limit=100):
    #     _logger.info('name_search =====> %s', name)
    #     if self._context.get('sale_show_partner_name'):
    ...
    #     return super(OctagonoGPS, self).name_search(name, args, operator, limit)
    ...
    #         super(OctagonoGPS, self)._init_column(column_name)
    #     else:
    ...
    #                     'res_id': self.id,
    #                 }
    #     return super(OctagonoGPS, self).get_access_action(access_uid)
...
class OctagonoGPSLine(models.Model):
    ...
    def _compute_invoice_status(self):
        ...
        precision = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        for line in self:
            if line.state not in ('registered', 'done'):
                line.invoice_status = 'no'
            elif not float_is_zero(line.qty_to_invoice, precision_digits=precision):
                line.invoice_status = 'to invoice'
            elif line.state == 'registered' and line.product_id.invoice_policy == 'order' and \
    ...
    def _compute_product_updatable(self):
        for line in self:
            if line.state in ['done', 'cancel'] or (
                            line.state == 'registered' and (line.qty_invoiced > 0 or line.qty_delivered > 0)):
    ...
    def _compute_invoice_amount(self):
        for line in self:
            ...
            line.amt_to_invoice = (total_sale_line - invoiced_amount_total) if line.state in ['registered',
                                                                                              'done'] else 0.0
    ...
    def _get_to_invoice_qty(self):
        ...
        """
        for line in self:
            if line.order_id.state in ['registered', 'done']:
    ...
    def write(self, values):
        ...
        result = super(OctagonoGPSLine, self).write(values)
        return result
